...
def gen_driver_kconfig(driver):
    with open(join(driver_path, "Kconfig"), 'w') as kfd:
        kfd.write("#\n# Automatically generated by 'make config'. don't edit\n#\n\n")
        kfd.write("menu \"Device Driver\"\n\n")

        for kind in driver:
            driver_name = driver[kind]
            file_name = join("driver", kind, "Kconfig")
            if os.path.isfile(join(root, file_name)):
                kfd.write("source \"driver/" + kind + "/Kconfig\"\n\n")
...
# generate driver Kconfig
driver_kind = get_path_dir(driver_path)
driver = {}
for kind in driver_kind:
    driver[kind] = get_path_dir(join(driver_path, kind))
gen_driver_kconfig(driver)
